...
dataname=`cat ${input_file} | yq -r '.GHC.dataset.name'`
fhirname=`cat ${input_file} | yq -r '.GHC.fhir_store_name.name'`
location=`cat ${input_file} | yq -r '.GHC.location.name'`
import_option=`cat ${input_file} | yq -r '.GHC.import_data.option'`
project_name=`cat ${input_file} | yq -r '.GHC.gcp_project.name'`
load_option=`cat ${input_file} | yq -r '.GHC.import_data.option'`
bucket_name=`cat ${input_file} | yq -r '.GHC.import_data.bucket_name'`
use_custom_proxy=`cat ${input_file} | yq -r '.GHC.custom_proxy.option'`
custom_proxy_name=`cat ${input_file} | yq -r '.GHC.custom_proxy.proxy'`
custom_product_name=`cat ${input_file} | yq -r '.GHC.custom_proxy.product'`
use_fhir_store_as_proxy=`cat ${input_file} | yq -r '.GHC.use_fhirstore_as_proxy_name'`
version=`cat ${input_file} | yq -r '.GHC.fhir_version'`
backupBucketName=`cat ${input_file} | yq -r '.GHC.backupBucketName'`
gpsub=`cat ${input_file} | yq -r '.GHC.pubsub_topic'`
gpsub=$(sed 's|/|\\/|g' <<< $gpsub)
up_create=`cat ${input_file} | yq -r '.GHC.enable_update_create'`
ref_integrity=`cat ${input_file} | yq -r '.GHC.disable_referential_integrity'`
res_versioning=`cat ${input_file} | yq -r '.GHC.disable_resource_versioning'`
hist_import=`cat ${input_file} | yq -r '.GHC.enable_history_import'`
google_api_version=`cat ${input_file} | yq -r '.GHC.google_api_version'`
...
proxyurl=`cat ${input_file} | yq -r '.GHC.proxyurl'`
nextpath="v1/${version}/"
...
service_account_key=`cat ${input_file} | yq -r '.GHC.service_account'`
