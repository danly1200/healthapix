try {
	...
	context.setVariable("patientIdInContext",patientId);
	...
	var patientResource = JSON.parse(context
			.getVariable("stu3.b2b.path.filter.map"))[resource];
	...
	if (patientResource) {
		...
		if (validPatientScopeFound) {
			if (patientId) {
				//context.setVariable("request.queryparam." + patientResource,patientId);
				context.setVariable("matchPatientInResponseWithContext",
						true);		
			...
			if (resourceId) {
				...
				throw "Resource ID is not available in the context";
			}
		} else {
