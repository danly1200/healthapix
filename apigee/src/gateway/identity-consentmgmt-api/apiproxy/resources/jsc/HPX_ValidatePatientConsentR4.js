...
if(consentResponse.total>0){
    ...
    var consentWindowStart = Date.parse(consentResponse.entry[0].resource.provision.period.start);
    var consentWindowEnd = Date.parse(consentResponse.entry[0].resource.provision.period.end);
    var currentDateTime = Date.now();
    if(consentWindowStart < currentDateTime){
        if(consentWindowEnd > currentDateTime){
            var consentedScopes = consentResponse.entry[0].resource.scope.coding[0].code;
